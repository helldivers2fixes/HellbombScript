...
"NoVegaGPUs" = @{
    ...
    'TestFailMsg' = @'
    Write-Host "$([Environment]::NewLine)[FAIL] " -ForegroundColor Red -NoNewLine
    Write-Host "Your system has a Vega-based AMD GPU & is not using driver version" -ForgroundColor Yellow -NoNewLine
...
$Title = @(
    "-------------------------------------------------------------------------------------------------------",
    "💣 Hellbomb 💣 Script for Troubleshooting Helldivers 2       ||      Version 3.63",
    "-------------------------------------------------------------------------------------------------------",
    $AdminBanner
) -join "`n"
