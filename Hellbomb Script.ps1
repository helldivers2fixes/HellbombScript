...
Function Test-ClientDnsConfig {
    ...
    If ((-not ([string]::IsNullOrEmpty($dnsServersIPv4))) -and ($dnsServersIPv4 | Get-Member -Name 'ServerAddresses')) {
        Write-Host "[PASS]" -ForegroundColor Green -NoNewline
    ...
    If ((-not ([string]::IsNullOrEmpty($dnsServersIPv6.ServerAddresses))) -and ($dnsServersIPv6 | Get-Member -Name 'ServerAddresses')) {
        Write-Host "[PASS]" -ForegroundColor Green -NoNewline
...
Function Test-MemoryChannels {
    ...
    Try { $DeviceLocatorChannels = ($SystemMemoryInfoObj.DeviceLocator | Sort-Object -Unique).Count }
    Catch {
            # If that errors, then there is only one DIMM installed
            $DeviceLocatorChannels = 1
    }
    Try { $BankLabelChannels = ($SystemMemoryInfoObj.BankLabel | Sort-Object -Unique).Count }
    Catch {
            # If that errors, then there is only one DIMM installed
            $BankLabelChannels = 1
    }
